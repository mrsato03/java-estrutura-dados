...
public class Vetor<T> {
	...
	public void inserir(T elemento) {
		if (this.posicao >= elementos.length) {
	...
	public void inserirEm(int posicao, T elemento) {
		...
		if (this.elementos[posicao] != null) {
			// 1, 2, 3, 4
			// 1, 5, 3, 4
			Object[] arrayFinal = Arrays.copyOfRange(this.elementos, posicao, tamanho());
	...
	public boolean contem(T valor) {
		for (int i = 0; i < tamanho(); i++) {
			T elem = recuperar(i);
			if (elem != null && elem.equals(valor)) {
				return true;
			}
		}
		return false;
	...
	public void remover(T elemento) {
		...
		if (posicao >= tamanho() || posicao == -1) {
			erroElemento(posicao);
		}
		remover(posicao);
	}
	
	private void erroPosicao(int posicao) {
		throw new IllegalArgumentException(String.format("Posição inváldia [%d]", posicao));
	}
	
	private void erroElemento(int posicao) {
